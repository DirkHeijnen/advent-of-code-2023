...
class Instruction {
     ...
     */
    constructor(line) {
        const color = line.split(' ')[2].trim().replaceAll('(', '').replaceAll(')', '');

        this.direction =  this.#getDirection(color);
        this.steps = this.#getSteps(color);
    }

    /**
     * Given the hexadecimal color (ex. #70c710), take the first five characters and convert to a base-10 number.
     * 
     * @param {String} color The hexadecimal color (ex. #70c710).
     * @returns {Number} The amount of steps it should take based on the given color.
     */
    #getSteps(color) {
        // Take first 5 characters.
        const firstFive = color.split('').slice(1, 6).join('');

        // Parse int can convert hex to base10.
        return parseInt(firstFive, 16);
    }

    /**
     * Given the hexadecimal color (ex. #70c710), take the final character.
     * 
     * If 0 then the instruction is to move right.
     * If 1 then the instruction is to move down.
     * If 2 then the instruction is to move left.
     * If 3 then the instruction is to move up.
     * 
     * @param {String} color The hexadecimal color (ex. "#70c710")
     * @returns {Direction} The direction enum for the given direction or undefined.
     */
    #getDirection(color) {
        const lastCharacter = color[color.length - 1];
