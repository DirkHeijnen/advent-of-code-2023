...
const main = () => {
    ...
    // Find loops.
    // Each loop has a direction and all the ordered steps of the loop.
    const loops = [
    ...
    // Get the longest loop.
    const longestLoop = orderedLoops[0];
    

    // Create the loop polygon.
    let polygon = longestLoop.steps.map((pipe) => [pipe.row, pipe.column]);
    let count = 0;


    // Check if points in inside polygon but not on the edge (the loop itself)
    for (let row = 0; row < pipeMaze.getRowCount(); row++) {
        for (let column = 0; column < pipeMaze.getColumnCount(); column++) {

            if (isPointInsidePolygon([row, column], polygon)) {

                let isOnEdge = false;

                for (let step of longestLoop.steps) {
                    let stepPosition = pipeMaze.getPosition(step.row, step.column);

                    if (stepPosition.equals(pipeMaze.getPosition(row, column))) {
                        isOnEdge = true;
                    }
                }
