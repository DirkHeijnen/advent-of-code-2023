...
class Almanac {
    ...
    constructor(text) {
        ...
        // 2. Extract the seeds
        let seedNumbers = textLines[0].replace('seeds: ', '').split(' ').map(x => parseInt(x));
        while (seedNumbers.length > 0) {
            const seedRangeStart = seedNumbers.shift();
            const seedRangeLength = seedNumbers.shift();
            const seedRangeEnd = seedRangeStart + seedRangeLength;
            this.seedRanges.push({ start: seedRangeStart, end: seedRangeEnd});
        }
...
const main = () => {
    ...
    // For each map in the almanac, add a pipeline step with the corresponding map.
    for (let map of almanac.transformationMaps) {
        pipeline.addStep(new PipelineStep(map));
    }

    // Define the location value for the lowest detected location.
    let location = Number.MAX_SAFE_INTEGER;

    for (let seedRange of almanac.seedRanges) {
        for (let i = seedRange.start; i < seedRange.end; i++) {
            let newLocation = pipeline.run(i);
    ...
    // Log the answer.
    console.log("The closest location = " + location);
}
